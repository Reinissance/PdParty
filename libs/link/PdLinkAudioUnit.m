...
+ (instancetype)defaultAudioUnit {
	return [[PdLinkAudioUnit alloc] initWithComponentDescription:PdLinkAudioUnit.defaultIODescription
                                                     options:0
                                                       error:nil];
}
...
- (instancetype)initWithComponentDescription:(AudioComponentDescription)componentDescription
	...
	if (self) {
        ...
        _blockSizeAsLog = log2int(_blockFrames);
	}
	return self;
}

- (id)initWithLinkRef:(ABLLinkRef)linkRef {
    self = [super initWithComponentDescription:PdLinkAudioUnit.defaultIODescription error:0];
    linkRef_ = linkRef;
...
- (int)configureWithSampleRate:(Float64)sampleRate
    ...
    outputLatency_ = (UInt32)(secondsToHostTime * [app.pureData calculateLatency]);
        tickTime_ = (UInt32)(secondsToHostTime * _blockFrames / _sampleRate);
...
static OSStatus audioRenderCallback(void *inRefCon,
	...
	PdLinkAudioUnit *pd = (__bridge PdLinkAudioUnit *)inRefCon;
	Float32 *auBuffer = (Float32 *)ioData->mBuffers[0].mData;
    
    ABLLinkSessionStateRef session_state = ABLLinkCaptureAudioSessionState(pd->linkRef_);
...
//			[PdBase processFloatWithInputBuffer:(float *)copy outputBuffer:(float *)copy ticks:1];
            ...
            UInt64 hostTimeAfterTick = inTimeStamp->mHostTime + pd->outputLatency_;
            hostTimeAfterTick += pd->tickTime_;
...
//		[PdBase processFloatWithInputBuffer:auBuffer outputBuffer:auBuffer ticks:ticks];
        ...
        UInt64 hostTimeAfterTick = inTimeStamp->mHostTime + pd->outputLatency_;
        int bufSizePerTick = pd->_blockFrames * pd->_outputChannels;
    ...
    ABLLinkCommitAudioSessionState(pd->linkRef_, session_state);
...
- (BOOL)initAudioUnitWithSampleRate:(Float64)sampleRate
                     ...
                     outputChannels:(int)outputChannels {
	[self clearAudioUnit];
	AudioComponentDescription ioDescription = PdLinkAudioUnit.defaultIODescription;
