...
@interface PdLinkAudioController ()
...
@property (nonatomic, strong, readwrite) PdLinkAudioUnit *audioUnit;
...
@implementation PdLinkAudioController {
...
- (void)setupWithAudioUnit:(PdLinkAudioUnit *)audioUnit {
	_mixWithOthers = YES;
...
- (instancetype)init {
	self = [super init];
	if (self) {
		[self setupWithAudioUnit:[PdLinkAudioUnit defaultAudioUnit]];
	}
	return self;
}
...
+ (BOOL)addSessionOptions:(AVAudioSessionCategoryOptions)options {
	...
	return [PdLinkAudioController setSessionOptions:options];
}
...
- (void)mediaServicesWereReset:(NSNotification *)notification {
	...
	if (self.active) {
		...
		[PdLinkAudioController setSessionOptions:_options];
...
- (void)updateSessionCategoryOptions {
	...
	if (options) {
		[PdLinkAudioController setSessionOptions:options];
	}
	_options = session.categoryOptions;
	_optionsChanged = NO;
}
